...
const onSubmit = async () => {
  ...
  const randomInt = [21, 30, 24];
  const getRandom = randomInt[Math.floor(Math.random() * randomInt.length)];
  const store = useFpStore();
  await store.fetchDetailProduct(getRandom);
  router.push({ name: "info-detail", params: { id: getRandom } });
};
</script>
